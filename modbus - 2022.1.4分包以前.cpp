...
#define ZD_LAN_SERVER_IP        "192.168.31.101"         //中电莱斯传数据本地服务器ip   "192.168.31.101"
...
int dev_init()
{
    ...
    //初始化网口1，作为客户端使用，与ISR进行有线方式的注册连接
    system("ifconfig eth1 192.168.2.235 netmask 255.255.255.0");
    sleep(2);
    //开始绑定：监听192.168.2.2，端口:2234
    fd_lan = socket(AF_INET, SOCK_STREAM, 0);    //创建套接字
                ...
                if(( n=read(sockfd,hj_njdata_rece,1500))==0){
                  ...
                  cout<<"《《《《《《《《《《《《    接收到南水所的数据  》》》》》》》》》》》》："<<hj_njdata_rece<<endl;
                   ...
                   cout<<"《《《《《《《《《《《《 当前的客户端连接数  》》》》》》》》》》》》："<<client_num<<endl; 
                    ...
                    recvnj_message.push(string(temp));   
                   // flag_nj=1;
...
void* get_newsensor_data(void *arg)
{
    ...
    int wdt_id=-1;
    wdt_id=g_CsoftwareWdt->RequestSoftwareWdtID(threadname,softdogTimeout);//当前此线程注册
    ...
    while(1) //一直读取数据
    {
        ...
        sprintf((char*)temp_data_str,\
        "QN=%s;ST=%s;CN=%s;PW=%s;MN=%s;Flag=%s;CP=&&Gps=%s;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf&&",\
        hj_QN.c_str(), hj_ST, hj_CN, hj_PW, hj_MN.c_str(),hj_Flag,accessdev_gps.c_str(),\ 
        sensor_so2_code,sensor_so2,sensor_o2_code,sensor_o2,sensor_ch4_code,sensor_ch4,sensor_no_code,sensor_no,sensor_nh3_code,sensor_nh3,\
        sensor_no2_code,sensor_no2,sensor_h2s_code,sensor_h2s,sensor_co2_code,sensor_co2,sensor_co_code,sensor_co,sensor_temper_code,sensor_temper,\
        ...
        /********************************************************************私有协议包的的封装*************************************************************************/      
        int length= (strlen(hj212pacet.c_str())+54);
        change(length,len_j212);
        int num=len_j212.length();
        if(num==4)
        {
        }else if(num==3)
        ...
        if(!shipsensor_message.empty()){                        //如果有无人船的数据消息，将两个包封装成一个06包
            char temp[1500]={0};
            strcpy(temp,(char*)shipsensor_message.front().c_str());
            cout<<temp<<endl;
            sprintf((char*)send_data_packet,"$06%s%s%s00%s%s%s010%s%s%s%s%s@",communicateType.c_str(),id.c_str(),net_id.c_str(),len_j212.c_str(),Isr_mac.c_str(),\
            mac.c_str(),accessdev_gps.c_str(),cpu_mem.c_str(),communicate_status,hj212pacet.c_str(),temp);
            cout<<"封包后："<<send_data_packet<<endl;
            transMessage.push(send_data_packet);
            shipsensor_message.pop();
            cout<<"无人船传感器数据与大气数据封包成功等待发送"<<endl;
        }else{
            sprintf((char*)send_data_packet,"$06%s%s%s00%s%s%s010%s%s%s%s@",communicateType.c_str(),id.c_str(),net_id.c_str(),\
            len_j212.c_str(),Isr_mac.c_str(),mac.c_str(),accessdev_gps.c_str(),cpu_mem.c_str(),
            communicate_status,hj212pacet.c_str());  
            transMessage.push(send_data_packet);
        }                                       
        if(!recvnj_message.empty()){
            ...
            sprintf((char*)hj_njdata_packet,"$06%s%s%s00%s%s%s010%s%s%s%s%s@",communicateType.c_str(),id.c_str(),net_id.c_str(),len_j212.c_str(),Isr_mac.c_str(),\
            mac.c_str(),accessdev_gps.c_str(),cpu_mem.c_str(),communicate_status,hj212pacet.c_str(),temp);
            recvnj_message.pop();
            transMessage.push(hj_njdata_packet);
            cout<<"南京接收到的数据已打包"<<endl;
            }
        ...
        if(ret>0)
        {
           if (FD_ISSET(deviceFd, &fdset))
           {
                ...
                if (ret <= 58)
                {
                    ...
                    cout<<"接收线程接收到的消息："<<recv_message<<endl;
                                    ...
                                    else if (communicate_method.substr(2, 1) == "1")
                                    {
                                       //使用有线发送，修改通信模块类型
                                        ...
                                        else
                                        {
                                            ...
                                            cout << "fd_lan: send_data_packet = " << send_data_packet << endl;
                                            cout << "fd_lan: ret = " << ret << endl;
                                            cout << "Trans success(lan) " << endl;
                                            cout << "包的长度" <<len_private1<< endl;
#endif
...
{ 
    ...
    struct monitor485 *monitor=new monitor485(&transMessage, g_CsoftwareWdt);                                  
    /*初始化一些必要的结构体*/
    ...
    else
    {
        ...
        vec=CM->getALLIfd();
        struct deviceRegist *Regist = new deviceRegist(g_CsoftwareWdt, &vec);
        ...
        pthread_join(tid_deviceRegist,NULL);          //等待注册线程正常退出
        //delete 
        ...
        pthread_create(&tid_monitor485,NULL,RS485_Monitor,monitor); //开启串口监听线程
        //获取南水所的数据
        sleep(1);
