...
{   
    ...
    uint8_t c;
    uint8_t d;
    //溶解氧、氨氮、铜离子、镉离子、蓝绿藻、叶绿素、电导率
    int slave_address[7] = { 1,2,3,4,5,6,7 };
    ...
    while (1) {
        ...
        case 0: //从设备1 溶解氧传感器
            ...
            if (regs == -1) {
                ...
                c = 0;
                d = 0;
                cout << "读取无人船传感器" << index_flag + 1 << "数据失败" << endl;
                sleep(4);
                break;
            }
        ...
        if (index_flag == 6) {
            ...
            printf("溶解氧：%f\n氨氮：%f\n铜离子：%f\n镉离子：%f\n蓝绿藻：%f\n叶绿素：%f\n电导率：%f\n",
                sensor_data[0], sensor_data[1], sensor_data[2], sensor_data[3], sensor_data[5], sensor_data[7], sensor_data[8]);
            printf("温度1：%f\n温度2：%f\n",
            ...
            //cout << "开始封装数据段" << endl;
            // 数据段  CP=&&数据区&&
            sprintf((char*)ship_getpacket, "QN=%s;ST=%s;CN=%s;PW=%s;MN=%s;Flag=%s;CP=&&Gps=%s;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf&&", \
                ...
                sensor_lanlz_code, lanlz, sensor_yels_code, yels, sensor_diandl_code, diandl);
            //cout << "数据段封装完成" << endl;
