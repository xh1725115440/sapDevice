    ...
    while(1)
    {
        ...
        if(ret>0)
        {
           if (FD_ISSET(deviceFd, &fdset))
           {
                ...
                if(ret == 0) {
                    cout<<"ISR Quit"<<endl;
                    cout<<"SAP Will Reboot"<<endl;
                    return 0; //直接退出此线程，等待监控线程超时，系统重启
                }
                if (ret <= 58)
                {
                    ...
                    }else if(recv_protocal == TIME_SEND){//更新时间戳消息
                        count = 0;
                        g_CsoftwareWdt->KeepSoftwareWdtAlive(wdt_id);
...
{ 
    ...
    //struct monitor485 *monitor=new monitor485(&transMessage, g_CsoftwareWdt);   //用于无人船时给注释掉                                
    /*初始化一些必要的结构体*/
    ...
    else
    {
        ...
        //pthread_create(&tid_monitor485,NULL,RS485_Monitor,monitor);   //开启串口监听线程   用于无人船时给注释掉
        //获取南水所的数据
        sleep(1);
