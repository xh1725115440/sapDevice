...
* 更新日期：2024年10月25日
* 编写人： 唐康宁
* 目标： 根据服务器解析06包GPS数据方法，对获取GPS数据的代码进行修改   line 1289 ， line 1293
*       s_lon = s_lon + " " + lon[0] + lon[1] + lon[2] + lon[3] + lon[4] + lon[6] + lon[7] + lon[8] + lon[9] + lon[10];   //经度
*       s_lat = s_lat + lat[0] + lat[1] + lat[2] + lat[3] + lat[5] + lat[6] + lat[7] + lat[8] + lat[9];   //纬度
...
#include <unistd.h>                     //  进程相关，如 fork() getpid() sleep()
#include <math.h>                       //  数学计算
    ...
    if (-1 == fd)
    {
        ...
        while (1)
        {
            ...
            if (-1 == flag)
            {
                ...
                //lon是经度 lat是纬度
                string s_lon = "";
                /* 代码更新：唐康宁，内容：根据服务器解析06包GPS数据方法，修改GPS获取形式  */
                s_lon = s_lon + " " + lon[0] + lon[1] + lon[2] + lon[3] + lon[4] + lon[6] + lon[7] + lon[8] + lon[9] + lon[10];   //经度
                //s_lon = s_lon + f_lon + lon[0] + lon[1] + lon[2] + lon[3] + lon[4]; //经度
                string s_lat = "";
                /* 代码更新：唐康宁，内容：根据服务器解析06包GPS数据方法，修改GPS获取形式  */
...
void* get_ship_data(void* arg)
{
    ...
    float sensor_Humid = -999;
    float sensor_tempAir = -999;
    float pm10 = -999, pm25 = -999, pm1 = -999;
    int regs = 0;
    int index_flag = 0;
    /*数据前缀编码*/
    // 水质传感器
    char sensor_temper_code[] = "w01010";   // 温度
    ...
    char sensor_NTU_code[] = "w01012";  // 浊度
    char sensor_PH_code[] = "w01001";   // PH
    
    //大气传感器
    char sensor_tempAir_code[] = "q00005";   // 环境温度
    char sensor_CO_code[] = "a21005";    // CO
    char sensor_O3_code[] = "a19002";   // O3
    char sensor_SO2_code[] = "a21026";  // SO2
    char sensor_NO2_code[] = "a21004";  // NO2
    char sensor_CO2_code[] = "a05001";       //CO2二氧化碳
    char sensor_Humid_code[] = "a01002";    // Humidity 湿度
    char sensor_Light_code[] = "a01004";    // 光照
    char sensor_NH3_code[] = "a21001";   // NH3
    char sensor_O2_code[] = "a19001";   // O2
    ...
    if (modbus_connect(ctx1) == -1) {
        ...
        cout << "无人船传感器连接成功!" << endl;
    }
    while (1) {
        cout << "开始获取无人船传感器" << index_flag + 1 << "数据" << endl;
        //一直更新GPS信息数据
        ...
        case 1: //从设备2 氨氮传感器
            ...
            break;
        case 2: //从设备3 叶绿素传感器
            regs = modbus_read_registers(ctx1, 9728, 4, tab_reg); //对应于读取温度和叶绿素值    DCBA //0x26 00 = 9728
            if (regs == -1) {
                fprintf(stderr, "%s\n", modbus_strerror(errno));
                a = 0;
                b = 0;
        ...
        case 7: //从设备8 四合一，湿度、温度、光照、CO2
            ...
            sensor_Humid = tab_reg[0];
            sensor_data[9] = sensor_Humid * 0.1;
            Humid = sensor_data[9];

            

            sensor_tempAir = tab_reg[1];
            sensor_data[10] = sensor_tempAir * 0.1;
            tempAir = sensor_data[10];
        ...
        if (index_flag == 10) {
            ...
            //cout << "开始封装数据段" << endl;
            // 数据段  CP=&&数据区&&
            sprintf((char*)ship_getpacket, "QN=%s;ST=%s;CN=%s;PW=%s;MN=%s;Flag=%s;CP=&&Gps=%s;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf&&", \
                hj_QN.c_str(), hj_ST, hj_CN, hj_PW, hj_MN.c_str(), hj_Flag, accessdev_gps.c_str(), sensor_temper_code, temp1, sensor_temper_code, temp2, \
                sensor_rongO_code, rongO, sensor_NH_code, NH, sensor_lanlz_code, lanlz, sensor_yels_code, yels, \
                sensor_diandl_code, diandl, sensor_NTU_code, NTU, sensor_PH_code, PH,\
                sensor_Humid_code, Humid, sensor_tempAir_code, tempAir, sensor_CO2_code, CO2, sensor_Light_code, Light,\
                sensor_NH3_code, NH3, sensor_O2_code, O2);
            //cout << "数据段封装完成" << endl;
            ...
            lanlz = -999;
            yels = -999;
            diandl = -999; NH3 = -999; O2 = -999;
            temp1 = -999, temp2 = -999; tempAir = -999;
        }
        index_flag++;
