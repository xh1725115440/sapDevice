...
{   
    ...
    //溶解氧、氨氮、铜离子、镉离子、蓝绿藻、叶绿素、电导率、CO2、PM、测深仪、COD、气象站、噪音
    ...
    char sensor_co2_code[] = "a05001";       //二氧化碳
    ...
    while (1) {
        ...
        if (index_flag == 12) {
            ...
            printf("溶解氧：%f mg/L\n氨氮：%f mg/L\n铜离子：%f μg/L\n镉离子：%f mg/L\n蓝绿藻：%f 万cells/mL\n叶绿素：%f mg/L\n电导率：%f μS/cm\nTDS：%f mg/L\n液位值：%f m\nCOD：%f mg/L\n浊度：%f NTU\n",
                sensor_data[0], sensor_data[1], sensor_data[2], sensor_data[3], sensor_data[5], sensor_data[7], sensor_data[8], sensor_data[16], sensor_data[13], sensor_data[14], sensor_data[15]);
            printf("水温1：%f ℃\n水温2：%f ℃\n",
            ...
            sprintf((char*)ship_getpacket, "QN=%s;ST=%s;CN=%s;PW=%s;MN=%s;Flag=%s;CP=&&Gps=%s;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;"
                "%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf;%s-Rtd=%010.3lf&&", \
                ...
                sensor_co2_code, co2, sensor_pm25_code, pm25, sensor_pm10_code, pm10, sensor_pm1_code, pm1, sensor_depth_code, depth, sensor_COD_code, COD, sensor_turb_code, turb, \
...
void* get_gassensor_data(void* arg)
{
    ...
    int slave_address[6] = { 1,2,3,4,11,12 };
    uint16_t tab_reg[512] = { 0 };
    unsigned char temp_data_str[1024] = {};
    int regs = 0;
    int index_flag = 0;
    ...
    float co2 = -999, humid = -999, temper = -999, pm1 = -999, pm25 = -999, pm10 = -999;
    //HJ212前缀编码
    char sensor_co_code[] = "a21005";
    ...
    {   //软件喂狗
        ...
        if (index_flag == 5) {
            ...
            cout << "获取一轮后的数据如下：" << endl;
            printf("CO：%f ppm\nO3：%f ppm\nSO2：%f ppm\nNO2：%f ppm\nPM1：%f ug/m3\nPM10：%f ug/m3\nPM25：%f ug/m3\n湿度：%f %RH\n温度：%f ℃\nCO2：%f ppm\n", \
                co, o3, so2, no2, pm1, pm10, pm25, humid, temper, co2);
            cout << "--------------------------------" << endl;

            /* 原生数据的封装 */
            // 数据段  CP=&&数据区&&        
            sprintf((char*)temp_data_str, \
                "QN=%s;ST=%s;CN=%s;PW=%s;MN=%s;Flag=%s;CP=&&Gps=%s;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf&&", \
                hj_QN.c_str(), hj_ST, hj_CN, hj_PW, hj_MN.c_str(), hj_Flag, accessdev_gps.c_str(), \
                sensor_co_code, co, sensor_o3_code, o3, sensor_so2_code, so2, sensor_no2_code, no2, \
                sensor_pm1_code, pm1, sensor_pm10_code, pm10, sensor_pm25_code, pm25, sensor_humid_code, humid, sensor_temper_code, temper, sensor_co2_code, co2);
            ...
            co2 = -999, humid = -999, temper = -999, pm1 = -999, pm25 = -999, pm10 = -999;
        }
        index_flag++;
        g_CsoftwareWdt->KeepSoftwareWdtAlive(wdtShipId); //软件喂狗
        printf("thread of sensor in ship feed dog success\n");
...
{ 
    ...
    else
    {
        ...
        pthread_create(&tid_getSensor, NULL, get_gassensor_data, g_CsoftwareWdt);
