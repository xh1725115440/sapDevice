...
int connect_nonb(int sockfd, const struct sockaddr *saptr, socklen_t salen, int nsec)
{
    int     flags, n, error;
    socklen_t   len;
    fd_set  rset, wset;  //long类型的数组    
    struct timeval  tval;

    // 设置 socket 为非阻塞
    if ((flags = fcntl(sockfd, F_GETFL, 0)) == -1) {   //获取文件打开方式
...
int dev_init()
{
    ...
    else
    {
        ...
        memset(&wifi_ser_addr, 0x0, sizeof(struct sockaddr_in));        //初始化
...
void* GET_GPS(void* arg)
{
    ...
    int fd;         //接收GPS数据的串口
    int i = 0;
    fd = open((const char*)dev, O_RDWR | O_NOCTTY); //参数O_NOCTTY不会使打开的文件成为该进程的控制终端，返回一个文件描述符
    if (-1 == fd)
    {
#ifdef DEBUG
...
int main()
{
    ...
    int ret;                    //错误参数保存
    int fd_max;                 //最大文件描述符
    fd_set fdset;               //文件描述符集合 long类型的数组
