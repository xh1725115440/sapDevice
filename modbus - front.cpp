...
int fd_num;                                             //文件描述符数量
string id = "FF", net_id = "0000", mac, s_req_fre, bt_mac = "";      //设备id，设备网络id，设备mac，问询频率
int monitor_time = 4;
string current_time = "";
char communicate_status[] = "0000";               //通信方式描述数组，顺序为 lora  wifi  LAN 蓝牙
int frequency = 0;
unsigned int hj_crc;                                    
...
*   设备初始化,，并建立相应的网络连接
*   lora blue wifi lan
*   LoRa、WiFi、蓝牙和 LAN 有线设备
*   @return  失败：-1  成功：初始化成功设备个数
*
*/
int dev_init()
{
    ...
    {
        device_id.push_back(fd_lora); //lora记录
        printf("Init success: lora. Device descriptor = %d\n", fd_lora);
        communicate_status[0] = '1'; //已经初始化
    ...
    sleep(10);
    system("udhcpc -i wlan0 -t 8 -n");                 //为WiFi设备获取ip地址进程，最多发送8个DHCP包，没有获得ip则退出
    sleep(5);
    //-----上述使得  WiFi 设备能够正确地连接到指定的网络并获取有效的 IP 地址----
...
#endif
            fd_wifi = fd_wifi_sock;//赋值给wifi设备描述符
    ...
    //初始化LAN南水所 有线  ---作为服务端
    //设备的ip
     system("ifconfig eth0 192.168.31.101 netmask 255.255.255.0");//system 命令设置以太网接口 eth0 的 IP 地址和子网掩码。
     sleep(2);
    ...
    else
    {
        ...
        Llan_ser_addr.sin_family = AF_INET; //ipv4协议族
        //svraddr.sin_addr.s_addr = INADDR_ANY;
        Llan_ser_addr.sin_addr.s_addr = inet_addr(ZD_LAN_SERVER_IP);  //inet_addr   192.168.31.101
        Llan_ser_addr.sin_port = htons(LAN_SERVER_PORT);  //指定目标机端口:2234
        bind(fdL_lan,(struct sockaddr *)&Llan_ser_addr,sizeof(Llan_ser_addr)); //绑定 ip和端口号 192.168.31.101
        listen(fdL_lan,5);
#ifdef DEBUG
    ...
    system("ifconfig eth1 192.168.2.2 netmask 255.255.255.0"); //system 命令设置以太网接口 eth1 的 IP 地址和子网掩码。
    sleep(2);
    ...
    else //创建套接字成功
    {
        ...
        lan_ser_addr.sin_family = AF_INET;  //ipv4协议族
        //svraddr.sin_addr.s_addr = INADDR_ANY;
        lan_ser_addr.sin_addr.s_addr = inet_addr(LAN_SERVER_IP); // "192.168.2.1" 
        lan_ser_addr.sin_port = htons(LAN_SERVER_PORT); //指定目标机端口:2234
#ifdef DEBUG
...
void device_regist()
{
    ...
    regist_message = regist_message + "$" + "01" + "1" + id + net_id + "00" + "0022" + mac + communicate_status +"010"+ gps_lon+gps_lat+"@";  //注册请求消息
        ...
        while (i--)
        {
            ...
            if (ret > 0)
            {
                ...
                if (recv_protocal == REQ_SEND_INFO)
                {
                    ...
                    {
                        //确认是该接入设备发起的注册请求
                        id = regist_recv_message.substr(32, 2);             //获取下发的id
                        net_id = regist_recv_message.substr(34, 4);          //获取所在网络的id
                        current_time = regist_recv_message.substr(38, 17);   //获取网关下发参数时间戳 2016 08 01 08 58 57 223
...
void Get_L_data(){
    fd_set rest,allset; 
    int client[64];//是为了保存已经建立连接并返回的新套接字描述符
...
int get_sensor_data()
{
    ...
    while (1)  //一直读取数据
    {
        ...
        int length= (strlen(hj_data_packet.c_str())+30);
        change(length);
...
int main()
{
    ...
    else
    {
        ...
        //获取sensor数据
        thread th_sensor_data(get_sensor_data);
        th_sensor_data.detach();

        //获取南水所的数据
        sleep(1);
        thread th_ll_data(Get_L_data);
        th_ll_data.detach();
        ...
        // #ifdef DEBUG
        //         cout << "打印消息队列================================start"<<endl;;
        //         cout <<" send_message.empty:"<< send_message.empty() << endl;
        //         while(send_message.empty()==false)
        //         {
        //             cout << "打印send_message消息:"<<send_message.front()<<endl;
        //             send_message.pop();
        //         }
        //        cout << "打印消息队列================================end"<<endl;
        // #endif


        //开启发送线程
        thread th_trans_message(trans_message);
        th_trans_message.detach();
        while (1);
    }
}
