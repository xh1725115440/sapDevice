...
int fd_lora = -1, fd_wifi = -1, fd_bt = -1, fd_lan = -1;//通信设备描述符
...
int connect_nonb(int sockfd, const struct sockaddr *saptr, socklen_t salen, int nsec)
{
    ...
    if ((n = select(sockfd + 1, &rset, &wset, NULL, nsec ? &tval : NULL)) == 0) {  //返回值是满足事件的总个数          //@@@这一段为什么要有
...
int dev_init()
{
    ...
    system("ifconfig eth0 192.168.2.2 netmask 255.255.255.0");
    sleep(2);
    ...
    while (1)
    {
        ...
        if (ret > 0)
        {
            ...
            if (FD_ISSET(fd_lora, &fdset))
            {
                ...
                // if (ret < 58 && RX_buf[0] != '$' && RX_buf[ret - 1] == '@') //该包为数据包
                // {
...
int get_sensor_data()
{
    ...
    while (1)  //一直读取数据
    {
        ...
        //数据段，原生数据
        sprintf((char*)temp_data_str, "QN=%s;ST=%s;CN=%s;PW=%s;MN=%s;Flag=%s;CP=&&%s-Rtd=%08d;%s-Rtd=%08d;%s-Rtd=%08d;%s-Rtd=%08d;%s-Rtd=%08d;%s-Rtd=%08d;%s-Rtd=%08d;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf;%s-Rtd=%08.1lf;%s-Rtd=%08.1lf;%s-Rtd=%08.2lf;%s-Rtd=%08.2lf&&", hj_QN.c_str(), hj_ST, hj_CN, hj_PW, hj_MN.c_str(), \
        ...
        len_private= (strlen(send_data_packet));
